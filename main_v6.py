...
MAX_ITER = 300            # Max number of generations
TEST_INTERVAL = 5        # Interval for testing/rendering the best genome
CURRENT_GENERATION = 0
RENDER_ENABLED = False  # Global flag to control visualization
...
def eval_genomes(genomes, config):
    ...
    for genome_id, genome in genomes:
        ...
        for episode in range(5):
            ...
            hit_count = 0
            time_with_ball = 0
            prev_other_lives = 5  # Track previous lives count
            ...
            while not done:
                ...
                if reward > 0:  # Ball was hit
                    ...
                    if distance <= TOTAL_RADIUS:
                        ...
                        if dx < 0:  # Front-side hit (ball is to the left of agent)
                            if ball_vx < -0.8:  # Strong hit towards opponent
                                hit_reward = 300.0
                ...
                # Get current lives
                other_lives = info['ale.otherLives']
                
                # Check if opponent lost a life (we scored)
                scoring_reward = 0
                if other_lives < prev_other_lives:
                    scoring_reward = 1000.0  # Big reward for scoring
                    prev_other_lives = other_lives
...
def render_best_genome(winner, config, num_episodes=1, save_gif=False, generation=None):
    env = gym.make("SlimeVolley-v0")
            ...
            while not done:
                if save_gif:
                    ...
                    if frame is not None:
                        frames.append(frame)
                elif RENDER_ENABLED:  # Only render if flag is True
                    env.render()
...
class FitnessPlotter:
    def __init__(self):
        ...
        self.mean_fitness = []
        self.max_fitness = []
        # Create and save an initial empty plot
        self.create_base_plot()
        
    def create_base_plot(self):
        """Create and save an initial empty plot"""
        plt.figure(figsize=(10, 6))
        plt.title('NEAT Training Progress', fontsize=14)
        plt.xlabel('Generation', fontsize=12)
        plt.ylabel('Fitness', fontsize=12)
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.savefig('fitness_curves.png', dpi=300, bbox_inches='tight')
        plt.close()
        
    def add_generation(self, generation: int, population: dict):
        """Record fitness statistics for a generation"""
        fitnesses = [genome.fitness for genome in population.values()]
        
        self.generations.append(generation)
        self.mean_fitness.append(float(np.mean(fitnesses)))
        self.max_fitness.append(float(np.max(fitnesses)))
        
    def update_plot(self):
        """Update the fitness curves plot"""
        plt.figure(figsize=(10, 6))
        
        # Plot mean fitness
        plt.plot(self.generations, self.mean_fitness, 'g-', label='Mean Fitness', linewidth=2)
        
        # Plot max fitness
        plt.plot(self.generations, self.max_fitness, 'm-', label='Max Fitness', linewidth=2)
        ...
        # Save the plot, overwriting the previous version
        plt.savefig('fitness_curves.png', dpi=300, bbox_inches='tight')
        plt.close()

# Modify your VisualizeBestGenomeReporter class
class VisualizeBestGenomeReporter(neat.reporting.BaseReporter):
    ...
    def post_evaluate(self, config, population, species, best_genome):
        ...
        self.fitness_plotter.add_generation(self.generation, population)
        
        # Update and save the plot at each test interval
        if self.generation % TEST_INTERVAL == 0:
            self.fitness_plotter.update_plot()
            print(f"\nUpdated fitness curves plot at generation {self.generation}")
            
            # Create SVG visualizations every TEST_INTERVAL generations
            try:
                # Generate statistics visualization
                visualize.plot_stats(self.stats, 
                                   ylog=False, 
                                   view=False,
                                   filename=f'visualizations/stats_gen_{self.generation}.svg')
                
                # Generate species visualization
                visualize.plot_species(self.stats,
                                     view=False,
                                     filename=f'visualizations/species_gen_{self.generation}.svg')
                
                # Generate network visualization
                create_network_visualization(config, 
                                          best_genome, 
                                          f'network_gen_{self.generation}')
                
                print(f"Created SVG visualizations for generation {self.generation}")
            except Exception as e:
                print(f"Warning: Failed to create SVG visualizations: {str(e)}")
        ...
        if self.generation % TEST_INTERVAL == 0:
            ...
            try:
                render_best_genome(best_genome, self.config, num_episodes=1, 
                                 save_gif=False, generation=self.generation)
            except Exception as e:
    ...
    def complete_extinction(self):
        ...
        self._save_final_visualizations()
    ...
    def _save_final_visualizations(self):
        """Helper method to save final visualizations"""
        try:
            visualize.plot_stats(self.stats, ylog=False, 
                               filename='visualizations/fitness_history_final.svg')
            visualize.plot_species(self.stats, 
                                 filename='visualizations/speciation_final.svg')
            self.fitness_plotter.save_plot('visualizations/fitness_curves_final.png')
        except Exception as e:
            print(f"Warning: Failed to save final visualizations: {str(e)}")
...
def run_neat(config_file):
    ...
    render_best_genome(winner, config, num_episodes=3, save_gif=False)
